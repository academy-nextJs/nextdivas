...
<svg
  ...
  <div className="flex-1 z-10 mx-4 flex flex-col justify-between text-right text-sm md:text-base mt-4 md:mt-0">
    ...
    <div className="flex justify-start gap-4 md:gap-8 mt-3 text-xs md:text-sm text-white opacity-90 ">
      <div className="flex items-center gap-1">
        {data.bathrooms ? toPersianDigits(data.bathrooms) : "۲"} حمام <FaBath />
      </div>
      <span>|</span>
      <div className="flex items-center gap-1">
        {data.parking ? toPersianDigits(data.parking) : "۴"} پارکینگ <FaCar />
      </div>
      <span>|</span>
      <div className="flex items-center gap-1">
  ...
  <div className="z-10 flex flex-wrap md:flex-col justify-between md:items-center items-start text-right mt-4 md:mt-0">
    ...
    <button className="hover:bg-[#7DF93A] hover:shadow-2xl border border-[#8CFF45] shadow-inner shadow-[#8CFF451F] text-[#8CFF45] hover:text-black text-xs md:text-sm font-bold w-full md:w-40 h-10 md:h-11 rounded-xl hover:brightness-110 transition-all">
      مشاهده ملک
    </button>
  </div>
</div>
