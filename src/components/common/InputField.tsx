      ...
      {type === "select" ? (
        <>
          <select
            ...
            className={`appearance-none bg-transparent peer grid border-2 border-gray-500 focus:outline-0 focus:border-white shadow-sm ${inputStyle}`}
          >
            {options?.map((option) => (
              <option key={option.value} value={option.value} className="bg-white text-black text-right">
                {option.label}
              </option>
            ))}
          </select>
